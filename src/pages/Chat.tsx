      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex h-14 items-center gap-2 border-b border-border px-3 sm:px-4">
          ...
          <select
            ...
            className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-content outline-none sm:hidden"
          >
            {channels.map((c) => (
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          ...
          {thread.length === 0 && (
            <p className="pt-10 text-center text-sm text-muted">No messages yet. Start the conversation!</p>
          )}
        </div>
        <form onSubmit={send} className="flex gap-2 border-t border-border p-3">
          <input
            ...
            className="h-10 flex-1 rounded-lg border border-border bg-bg px-3 text-sm text-content focus:border-secondary focus:outline-none"
          />
          <button
            type="submit"
            className="grid h-10 w-10 place-items-center rounded-lg bg-primary text-white hover:bg-secondary"
          >
            <Send size={16} />
          </button>
        </form>
