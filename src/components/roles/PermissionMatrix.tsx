  ...
  return (
    <>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              ...
              {roles.map((role) => (
                <th key={role} className="px-3 py-3 text-center text-xs font-medium text-muted">
          ...
          <tbody>
            {permissions.map((permission, index) => (
              <tr key={permission} className="border-b border-border last:border-0">
                <td className="px-4 py-3 font-medium text-content">{permission}</td>
                {roles.map((role) => (
      ...
      <p className="mt-3 text-xs text-muted">
        Permissions must also be enforced in the backend - never trust frontend checks alone.
      </p>
    </>
  );
}
