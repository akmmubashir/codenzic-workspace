  ...
  return (
    <div className="space-y-4">
      {announcements.map((announcement) => {
        const author = empById(announcement.authorId);
        return author ? (
          <Card key={announcement.id} className="p-5">
            <div className="mb-2 flex items-center gap-2">
              {announcement.pinned && <Pin size={14} className="text-primary" />}
              <Badge label={announcement.category} />
              <span className="text-xs text-muted">
                {new Date(announcement.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
              </span>
            </div>
            <h3 className="text-lg font-semibold text-content">{announcement.title}</h3>
